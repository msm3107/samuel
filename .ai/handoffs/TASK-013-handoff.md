...
the owner's (Mikołaj Smoliniec, 2026-09-22); eight more are proposed and
await the owner.
...
4. **`hostname` and `unicodeHostname`** (owner), the second from Node's
   ...
   differ (PR #26, note 3).
5. **Only TASK-012's output is stored** (proposed). Zod checks the body's
...
6. **An `aiSystemId` from nowhere, or from another organization, is 404
   `ai_system_not_found`** (proposed), from the composite foreign key. Both
   cases get the same answer.
7. **409 `deployment_exists`** on create or restore (proposed), from
   TASK-011's unique index; only the caller's own system is compared.
8. **PATCH is `{ status }` only, with no version** (proposed). A single
   switch has nothing to overwrite; a clashing restore is refused by the
   database.
9. **Response fields** (proposed): `id`, `aiSystemId`, `aiSystemStatus`,
   `hostname`, `unicodeHostname`, `status`, `createdAt`, `updatedAt`.
10. **Lists as for AI systems** (proposed): by hostname, capped at 200, with
    `truncated`. A filter naming a foreign or missing system lists nothing.
11. **No audit calls in the application** (proposed): TASK-011's triggers
    write them, without the hostname.
12. **The hostname is never logged** (proposed); refusals log their code.
...
- **TASK-015 must show both hostname forms** when they differ, and mark a
  deployment of an archived system as inactive.
...
- **The archived-system refusal is matched by its message.** If TASK-011's
  ...
  real-database test would catch that.
